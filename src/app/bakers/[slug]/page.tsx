...
export default async function BakerProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let query = supabase
    ...
    .select("user_id, slug, business_name, bio, city, postal_code, country_code, website, instagram_handle, delivery_radius_km, minimum_lead_days, starting_price, currency, is_verified, accepting_orders")
    .eq("status", "active")
    .eq("is_discoverable", true);

  query = UUID_PATTERN.test(slug) ? query.eq("user_id", slug) : query.eq("slug", slug);
